...
public class GenerateExpenseReportPdfUseCase : IGenerateExpenseReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        foreach (var expense in expenses)
        {
            ...
            SetStyleBaseForExpenseInformation(row.Cells[0], expense.CreatedDate.ToString("D"));
            row.Cells[0].Format.LeftIndent = 20;

            SetStyleBaseForExpenseInformation(row.Cells[1], expense.CreatedDate.ToString("t"));

            SetStyleBaseForExpenseInformation(row.Cells[2], expense.PaymentType.PaymentTypeToString());
    ...
    private Document CreateDocument(DateOnly month)
    {
        ...
        document.Info.Author = "Milton Soares";
    ...
    private void CreateHeaderWithProfilePhotoAndName(Section page)
    {
        ...
        var headerTable = page.AddTable();
        headerTable.AddColumn();
        headerTable.AddColumn("300");

        var row = headerTable.AddRow();
        var assembly = Assembly.GetExecutingAssembly();
        var directoryName = Path.GetDirectoryName(assembly.Location);
        var filePath = Path.Combine(directoryName!,"UseCases", "Expenses", "Reports", "Pdf", "Logo", "96bits.png");

        row.Cells[0].AddImage(filePath);
        row.Cells[1].AddParagraph("Hey, Milton Soares");
    ...
    private void CreateTotalSpentSection(Section page, DateOnly month, decimal totalExpenses)
    {
        ...
        paragraph.AddFormattedText(
            text: title,
            font: new Font { Name = FontHelper.RALEWAY_REGULAR, Size = 15 }
        );

        paragraph.AddLineBreak();

        paragraph.AddFormattedText(
            text: $"{CURRENCY_SYMBOL} {totalExpenses}",
            font: new Font { Name = FontHelper.WORKSANS_BLACK, Size = 32 }
        );
    }
    ...
    private void AddExpenseTitle(Cell cell, string expenseTitle)
    {
        ...
        cell.Format.Font = new Font { Name = FontHelper.RALEWAY_BLACK, Size = 16, Color = ColorsHelper.BLACK };
    ...
    private void AddHeaderForAmount(Cell cell)
    {
        ...
        cell.Format.Font = new Font { Name = FontHelper.RALEWAY_BLACK, Size = 16, Color = ColorsHelper.WHITE };
        cell.Shading.Color = ColorsHelper.RED_DARK;
        cell.VerticalAlignment = VerticalAlignment.Center;
        cell.Format.RightIndent = 20;
    }
    
    private void SetStyleBaseForExpenseInformation(Cell cell, string rowText)
    {
        cell.AddParagraph(rowText);
